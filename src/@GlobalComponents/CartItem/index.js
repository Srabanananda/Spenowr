...
import {
  addRemoveCartItemQty,
  deleteCartItem,
} from "../../@Endpoints/Core/Tabs/Shop";
...
const {
  DUMMY_IMAGE_URL,
  NEW_IMG_BASE,
  COLOR: { SUBNAME, APP_PINK_COLOR, LIGHTGREY, APP_THEME_COLOR },
} = Config;
...
const CartItem = ({ ...props }) => {
  ...
  console.log('CartItem render:', { title, product_type, primary_thumbnail_image, product_image, item });
  var productInfo = {};
  ...
  const onAddRemove = (type) => {
    ...
    addRemoveCartItemQty(body)
      .then((res) => {
        ...
        if (response_msg === "More quantity not available!")
  ...
  const getDeliveryInfo = () => {
    switch (delhivery_status) {
      case "1":
        return `Order Placed on ${dateFormatting(created_date)}`;
  ...
  const getDisplayImage = () => {
    if (product_type === "print-product") {
      // For print-product, always use product_image as background, fallback to primary_thumbnail_image
      const imageSwitch = product_image || primary_thumbnail_image;
      return (
        <CustomImageView
      ...
      // For all other types, just show the image
      const imageSwitch = primary_thumbnail_image || product_image;
      return (
        <Image
          source={{ uri: NEW_IMG_BASE + imageSwitch }}
          style={styles.Image}
        />
      );
    }
    ...
    >
      ...
      <View style={styles.detailWrapper}>
        ...
        <Text numberOfLines={1} style={styles.sellerText}>
          By {seller_name} ({product_type ? Capitalize(product_type) : ""})
        </Text>
        {isInactive ? (
          <Text numberOfLines={1} style={styles.qtyText2}>
            Quantity: {quantity}
          </Text>
        ) : null}
        <View style={styles.priceBox}>
          <Text style={styles.productPrice}>
            {getCurrency(JSON.stringify(currency))}
            {product_type === "service"
              ? original_price
              : cart_product_price || product_price}
          </Text>
          {cart_original_price && off && product_type !== "service" ? (
        ...
        {tracking_id ? (
          <Text style={styles.sellerText}>Tracking ID : {tracking_id}</Text>
        ) : null}
        {courier_info ? (
          <Text style={styles.sellerText}>
            Shipping Partner : {courier_info}
          </Text>
        ) : null}
        {delhivery_status ? (
          <Text style={styles.sellerText}>{getDeliveryInfo()}</Text>
        ) : null}
        {off && product_type !== "service" ? (
          <View style={styles.offView}>
            <Text style={styles.textOff}>{`${off}% off`}</Text>
          </View>
        ) : null}
        {isInactive || product_type === "service" ? null : (
          <View style={styles.actionWrapper}>
            <TouchableOpacity
              onPress={() => onAddRemove("remove")}
...
const style = (APP_PINK_COLOR, APP_THEME_COLOR, font_text_color) =>
  StyleSheet.create({
    containerWrapper: {
      flexDirection: "row",
