...
import { razorpaySuccessBuyJobPoints, razorpayFailedBuyJobPoints, processVerifyPaymentBuyJobPoints } from '../../../@Endpoints/Core/Tabs/Shop';
...
const RazorPayBuyJobPoints = (props) => {
  ...
  const successPaymentAPI = async (orderID) => {
    try {
      const res = await razorpaySuccessBuyJobPoints(orderID); // Assuming successPaymentAPI is in shopActions
  ...
  const callServerVerification = async (orderID) => {
    ...
    setLoader(true); // Start loader before API call
      const res = await processVerifyPaymentBuyJobPoints(orderID);
  ...
  const callFailedVerification = async (orderId) => {
    await razorpayFailedBuyJobPoints(orderId)
      .then(res => {
        console.log("=====res===>", res);
        setIsVerified(false);
      })
...
export default RazorPayBuyJobPoints;
