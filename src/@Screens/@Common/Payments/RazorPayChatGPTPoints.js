...
import { razorpaySuccessChatGPT, processVerifyPaymentChatGPT, razorpayFailedChatGPT } from '../../../@Endpoints/Core/Tabs/Shop';
...
const RazorPayChatGPTPoints = (props) => {
  ...
  const successPaymentAPI = async (orderID) => {
    try {
      const res = await razorpaySuccessChatGPT(orderID); // Assuming successPaymentAPI is in shopActions
  ...
  const callServerVerification = async (orderID) => {
    ...
    setLoader(true); // Start loader before API call
      const res = await processVerifyPaymentChatGPT(orderID);
  ...
  const callFailedVerification = async (orderId) => {
    await razorpayFailedChatGPT(orderId)
      .then(res => {
        console.log("=====res===>", res);
        setIsVerified(false);
      })
...
export default RazorPayChatGPTPoints;
