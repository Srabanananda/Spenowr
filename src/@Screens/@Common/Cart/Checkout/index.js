...
import { calculatePrice } from "../../../../@Utils/helperFiles/helpers";
import { SafeAreaView } from "react-native-safe-area-context";

const {
  COLOR: { LIGHTGREY, SUBNAME, WHITE },
} = Config;

const ShoppingCartCheckoutScreen = ({ ...props }) => {
  const { navigation, userObj, shippingAddresses, route } = props;
  ...
  console.log('====================================');
  console.log('shippingAddresses 58',shippingAddresses[0]?.country);
  console.log('====================================');

  const defaultAddress = shippingAddresses.findIndex(
    (x) => x.shipping_id === default_address
  );
  console.log('====================================');
  console.log('defaultAddress 61',defaultAddress);
  console.log('====================================');
  ...
  const handleAddressChange = (index) => {
    ...
    console.log('====================================');
    console.log('indexindex90',index);
    console.log('====================================');
  };
  ...
  const validateData = () => {
    try {
      ...
      // }
    console.log('====================================');
    console.log('country.current 155',country.current);
    console.log('====================================');
      setPayLoader(true);
      const { shipping_id = "", contact_number = "" } =
      ...
      const paymentLinkObj = {
        ...
        product_info: "",
        country: country.current == '' ? shippingAddresses[0]?.country : country.current,
      };
  console.log('====================================');
  console.log('country.current 184',country.current);
  console.log('====================================');
      console.log('====================================');
      ...
      getPaymentLink(paymentLinkObj)
        .then((res) => {
          ...
          if (payment_list && !isObjectEmpty(payment_list)) {
            navigation.navigate("RazorPayPayment", {
              payload: {
                ...payment_list,
                customer_details: {
                  ...payment_list.customer_details,
                  customer_email: email,
                },
              },
            });
          } else {
            Toast.show("Fetching Payment List failed");
          }
        })
        .catch((e) => {
  ...
  const getRewardContent = (currency, amount) => {
    if (amount === "0.00")
      return <Text style={styles.availReward}> No Rewards available </Text>;
    return (
      <Text style={styles.availReward}>
        Reward Available : {getCurrency(currency)}{amount}{" "}
      </Text>
    );
  };
  ...
  const getData = () => {
    ...
    totalOriginalPrice = calculatePrice(addtocart);
    return (
      <ScrollView
        refreshControl={
          <RefreshControl
      ...
      >
        <View style={styles.container}>
          <Address
            ...
            refreshData={callApi}
            setAddressIndexCallback={handleAddressChange}
            onEnd={onModalClose}
          />
          ...
          <View style={[GlobalStyles.primaryCard, styles.detailContainer]}>
            ...
            <View style={styles.detailInnerBox}>
              <Text style={styles.detailText}>
                Price ({addtocart.length + " Items"})
              </Text>
              <Text style={styles.detailValue}>
                {getCurrency(mycurr)}{total_price_witout_shipping_price}
                {"  "}
                <Text style={styles.originalPrice}>
                  {getCurrency(JSON.stringify(total_currency))}
                  {totalOriginalPrice}
                </Text>
              </Text>
            </View>
            <View style={[styles.detailInnerBox, { borderWidth: 0 }]}>
              ...
              <Text style={styles.detailValue}>
                {getCurrency(mycurr)}{discountAmount}
              </Text>
            </View>
            <View style={styles.detailInnerBox}>
              <Text style={styles.detailText}>Shipping Fee</Text>
              <Text style={styles.detailValue}>
                {getCurrency(mycurr)}{total_shipping_price}
              </Text>
            </View>
            <View style={styles.detailInnerBox}>
              <Text style={styles.detailText}>Reward Balance Redeemed</Text>
              <Text style={styles.detailValue}>
                {getCurrency(mycurr)}{reedem_ammount}
              </Text>
            </View>
            <View style={styles.detailInnerBox}>
              <Text style={styles.detailText}>Total</Text>
              <Text style={styles.detailValue}>
                {getCurrency(mycurr)}{total_price}
              </Text>
            </View>
            <DefaultButton
              buttonText={"Pay Now"}
              onPress={() => validateData()}
              showLoader={payLoader || internalLoader}
            />
          </View>
        </View>
      </ScrollView>
  ...
  return (
    <SafeAreaView edges={['left', 'right']} style={GlobalStyles.GlobalContainer}>
      <DefaultHeader
        headerText={"Cart Checkout"}
        showCurr={true}
        getSelectedCurrency={setCurrency}
      />
