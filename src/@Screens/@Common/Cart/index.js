...
import { Text, ScrollView, StyleSheet, View, TouchableOpacity, RefreshControl } from "react-native";
...
const CartScreen = ({ ...props }) => {
  ...
  const getData = () => {
    ...
    const totalOriginalPrice = calculatePrice(addtocart);
    return (
      <ScrollView
        refreshControl={
          <RefreshControl
      ...
      >
        <View style={styles.container}>
          {addtocart.map((item, i) => (
            <CartItem
              containerStyles={{ marginBottom: moderateScale(10) }}
              item={item}
              key={i}
              refreshData={callApi}
            />
          ...
          <View style={[GlobalStyles.primaryCard, styles.detailContainer]}>
            ...
            <View style={styles.detailInnerBox}>
              ...
              <Text style={styles.detailValue}>
                {getCurrency(JSON.stringify(total_currency))}{Number(total_price_without_shipping).toFixed(2)}
                {"  "}
                <Text style={styles.originalPrice}>
                  {getCurrency(JSON.stringify(total_currency))}{Number(totalOriginalPrice).toFixed(2)}
                </Text>
              </Text>
            </View>
            <View style={styles.detailInnerBox}>
              ...
              <Text style={styles.detailValue}>
                {getCurrency(JSON.stringify(total_currency))}{Number(total_shipping_price).toFixed(2)}
              </Text>
            </View>
            <View style={styles.detailInnerBox}>
              <Text style={styles.detailText}>Total</Text>
              <Text style={styles.detailValue}>
                {getCurrency(JSON.stringify(total_currency))}{Number(total_price).toFixed(2)}
              </Text>
            </View>
            <DefaultButton
  ...
  return (
    <SafeAreaView edges={['left', 'right']} style={GlobalStyles.GlobalContainer}>
      ...
      >
        ...
        >
          <Text style={GlobalStyles.seeMoreButtonTextRev}>
            Continue Shopping
          </Text>
        </TouchableOpacity>
      </DefaultHeader>
