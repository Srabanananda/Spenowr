...
const Card = ({ ...props }) => {
    ...
    const getImage = () => {
        ...
        if (type === '') {
            return <ScaledImage source={{ uri: cover }} />;
        }
        if (image_path !== '') {
            return (
                <>
    ...
    return (
        <View style={styles.cardBox}>
            ...
            {url != null && (url.includes('.mp3') || url.includes('.mp4')) && 
                <DefaultButton buttonText='Play' onPress={()=>navigation.navigate('SeriesPlayer', { songs : songs })} />
            }
            <View style={styles.infoContainer}>
                ...
                {(article_poly_description && article_poly_description !== '') ? <Text style={styles.desc}>{article_poly_description}</Text> : null}
                {renderPricing()}
