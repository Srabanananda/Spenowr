...
import { StyleSheet, ScrollView, View, Text, TextInput, TouchableOpacity, Image, Platform, Alert } from "react-native";
...
const AddArtWorkScreen = ({ ...props }) => {
  ...
  const { subscription, points, animate_point } = route.params;
  ...
  useEffect(() => {
    if (EditData) {
      ...
      if (primaryImg.includes('https')) {

        setPrimaryImg(media_path);
      }
      else {
        setPrimaryImg(NEW_IMG_BASE + media_path);
      }

      if (secondary1.includes('https')) {
        setSecondary1(secondary_media1);
      }
      else {
        setSecondary1(NEW_IMG_BASE + secondary_media1);
      }

    if (secondary2.includes('https')) {
        setSecondary2(secondary_media2);
      }
      else {
        setSecondary2(NEW_IMG_BASE + secondary_media2);
      }
      setTag(photo_tag);
...
console.log('primaryImg 210',primaryImg);
      ...
      return (
        <TouchableOpacity onPress={() => chooseFile(option)}>
          <Image
            resizeMode={"contain"}
            source={{
              uri: typeof primaryImg == 'string' ? primaryImg : primaryImg.uri
            }}
      ...
      return (
        <TouchableOpacity onPress={() => chooseFile(option)}>
          <Image
            ...
            source={{
                uri: typeof secondary1 == 'string' ? secondary1 : secondary1.uri
              }}
            style={styles.selectedImageStyle}
          />
        </TouchableOpacity>
      ...
      return (
        <TouchableOpacity onPress={() => chooseFile(option)}>
          <Image
            ...
            source={{
                uri: typeof secondary2 == 'string' ? secondary2 : secondary2.uri
              }}
            style={styles.selectedImageStyle}
          />
        </TouchableOpacity>
  ...
  const chooseFile = (option) => {
    pickImage((res) => {
        ...
        console.log('response 265',response);
        // Update the state based on the platform and option
        switch (option) {
            case 0:
