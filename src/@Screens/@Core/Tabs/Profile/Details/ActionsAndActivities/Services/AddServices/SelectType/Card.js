...
const Card = ({cardItem,EditData,handleChange,data,type='large'}) =>{

    const [selected, setSelected] = useState(false);
    useEffect(()=>{ 
        if(EditData)
        {
            const res = data.find(x=>x.name === cardItem.name);
    ...
    const action = () =>{
        if(selected)
        {
            ...
            else index = data.findIndex(x => x.name === cardItem.name );
            data.splice(index,1);
            handleChange(data);
        }
        else
    ...
    if(type === 'small')
        return(
            <TouchableOpacity onPress={()=>action()} style={{...styles.smallBox,backgroundColor:selected ? APP_PINK_COLOR : WHITE}}>
                <Text style={{...styles.cardName, color: selected ? WHITE : DARK_BLACK}}>{cardItem.value}</Text>
            </TouchableOpacity>
        );
    return(
